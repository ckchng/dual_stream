...
class SegTrainer(BaseTrainer):
        ...
        for cur_itrs, (images, masks) in enumerate(pbar):
            ...
            if config.use_aux:
                with amp.autocast(enabled=config.amp_training):
                    ...
                    # print(f"[DEBUG] step {cur_itrs}: NaN/Inf in INPUT images")
                    # print("  images min / max:", images.min().item(), images.max().item())

                    # # if torch.isnan(masks).any() or torch.isinf(masks).any():
                    # print(f"[DEBUG] step {cur_itrs}: NaN/Inf in LABELS")
                    # print("  labels unique:", masks.unique())

                    # # if torch.isnan(preds).any() or torch.isinf(preds).any():
                    # print(f"[DEBUG] step {cur_itrs}: NaN/Inf in PREDICTIONS BEFORE LOSS")
                    # print("  preds shape/ min / max:", preds.shape, preds.min().item(), preds.max().item())
                    # If you have logits for aux head:
                    # if preds_aux is not None:
                    #     print("  preds_aux min / max:", preds_aux.min().item(), preds_aux.max().item())
                    # print("  labels unique:", masks.unique())
                    
                    

                    loss = _loss_fn(preds, masks)
                ...
                for i in range(len(preds_aux)):
                    ...
                    with amp.autocast(enabled=config.amp_training):
                        loss += config.aux_coef[i] * _loss_fn(preds_aux[i], masks_aux)
            ...
            # Detail loss proposed in paper for model STDC
                ...
                with amp.autocast(enabled=config.amp_training):
                    ...
                    #     print("  preds_aux min / max:", preds_aux.min().item(), preds_aux.max().item())
                    # print("  labels unique:", masks.unique())
                    # stop early so you see this batch clearly
                    # raise RuntimeError("NaN or Inf in preds before loss")
                    
                    
                    loss = _loss_fn(preds, masks)
                    # print('trainng with no aux')
                    # ===== DEBUG BLOCK END =====
    ...
    def validate(self, config, val_best=False):
        ...
        score = iou if iou.ndim == 0 else iou.mean()  # scalar for binary, mean for multiclass
            ...
            if config.use_tb and self.cur_epoch < config.total_epoch:
                ...
                if iou.ndim > 0:
                    for i in range(config.num_class):
                        self.writer.add_scalar(f'val/IoU_cls{i:02d}', iou[i].cpu(), self.cur_epoch+1)
        self.metrics.reset()
        return score
    ...
    def predict(self, config):
        ...
        for (images, images_aug, img_names) in tqdm(self.test_loader):
            ...
            if config.num_class == 1:
                ...
                masks = (probs > config.pred_threshold).long().squeeze(1)
                preds = self.colormap[masks].cpu().numpy()
            else:

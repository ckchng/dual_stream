...
HARD_CONFIG = {
    ...
    "input_dir": '/home/ckchng/Documents/realtime-semantic-segmentation-pytorch-main/save/bg_50_no_crop/gray_rt_288_snr_1_15_new_bg_longer_dimmer/rt_map_only/two_stage_with_sep_rc_ignore_border/vis/tile_fp/',
    "non_overlap_dir": '/home/ckchng/Documents/realtime-semantic-segmentation-pytorch-main/save/bg_50_no_crop/gray_rt_288_snr_1_15_new_bg_longer_dimmer/rt_map_only/two_stage_with_sep_rc_ignore_border/vis/tile_fp/non_overlap_labels.txt',
    # Example: "fp_labels.txt". Leave as None to use CLI -o.
    "output_path": "/home/ckchng/Documents/realtime-semantic-segmentation-pytorch-main/save/bg_50_no_crop/gray_rt_288_snr_1_15_new_bg_longer_dimmer/rt_map_only/two_stage_with_sep_rc_ignore_border/vis/fp_labels.txt",
    # Window title
...
def main() -> int:
    ...
    non_overlap_path = Path(HARD_CONFIG.get("non_overlap_dir")) if HARD_CONFIG.get("non_overlap_dir") else None
    images: List[Path] = []
    if non_overlap_path and non_overlap_path.exists():
        # File expected to contain lines like: "img_name no_label" (whitespace separated)
        with non_overlap_path.open("r", encoding="utf-8") as nf:
            for line in nf:
                line = line.strip()
                if not line:
                    continue
        ...
        if not images:
            print(f"No image entries found in non-overlap file: {non_overlap_path}", file=sys.stderr)
            return 1
    else:

...
def log_config(config, logger):
    keys = ['task', 'dataset', 'num_class', 'model', 'encoder', 'decoder', 'loss_type',
            ...
            'DDP', 'kd_training', 'synBN', 'use_ema', 'use_aux', 'lambda_s2']

    config_dict = vars(config)
    infos = f"\n\n\n{'#'*25} Config Informations {'#'*25}\n" 
    infos += '\n'.join('%s: %s' % (k, config_dict.get(k, 'N/A')) for k in keys)
    infos += f"\n{'#'*71}\n\n"
    logger.info(infos)
...
def get_colormap(config):
    ...
    # For binary/single-class setups we still want two colors (bg/fg) to visualize masks.
    if config.num_class == 1 and len(colormap) < 2:
        colormap = [(0, 0, 0), (255, 255, 255)]

    if len(colormap) < config.num_class:
        raise ValueError('Length of colormap is smaller than the number of class.')
    else:
        return colormap[:max(config.num_class, 2 if config.num_class == 1 else config.num_class)]
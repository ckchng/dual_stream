...
class LearnedAnisoDownsample(nn.Module):
    """
    Input:  (B, C, 51, 180)
    Output: (B, C_out, 51, 23)
    """
    def __init__(self, in_ch, out_ch, mid_ch=None):
        super().__init__()
        mid_ch = mid_ch or out_ch

        self.stem   = ConvBNAct(in_ch, mid_ch, kernel_size=3, stride=(1,1), dilation=1, groups=1, bias=False, act_type='relu')

        # width downsample: 180 -> 90 -> 45 -> 23 (stride 2 in W only)
        self.down_w1 = ConvBNAct(mid_ch, mid_ch, kernel_size=3, stride=(1,2), dilation=1, groups=1, bias=False, act_type='relu')
        self.down_w2 = ConvBNAct(mid_ch, mid_ch, kernel_size=3, stride=(1,2), dilation=1, groups=1, bias=False, act_type='relu')
        self.down_w3 = ConvBNAct(mid_ch, mid_ch, kernel_size=3, stride=(1,2), dilation=1, groups=1, bias=False, act_type='relu')
        # pre-resize mixing (learnable)
        self.mix = ConvBNAct(mid_ch, mid_ch, kernel_size=3, stride=(1,1), dilation=1, groups=1, bias=False, act_type='relu')

        # post-resize “learned resampler”
        # self.post = nn.Sequential(
        #     ConvBNAct(mid_ch, out_ch, kernel_size=3, stride=(1,1), dilation=1, groups=1, bias=False, act_type='relu'),
        #     ConvBNAct(out_ch, out_ch, kernel_size=3, stride=(1,1), dilation=1, groups=1, bias=False, act_type='relu'),
        # )

    def forward(self, x):
        x = self.stem(x)       # (B, mid, 51, 180)
        x = self.down_w1(x)    # (B, mid, 51, 90)
        x = self.down_w2(x)    # (B, mid, 51, 45)
        x = self.mix(x)

        return x

# Depth-wise convolution block intended for anisotropic downsampling
class DWDownsample(nn.Sequential):
    def __init__(self, in_channels, out_channels=None, kernel_size=3, stride=(2, 2),
                 dilation=1, act_type='relu', **kwargs):
        if isinstance(kernel_size, list) or isinstance(kernel_size, tuple):
            padding = ((kernel_size[0] - 1) // 2 * dilation, (kernel_size[1] - 1) // 2 * dilation)
        elif isinstance(kernel_size, int):
        ...
        # Default to depth-wise downsample without channel change unless specified

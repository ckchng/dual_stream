...
class BaseTrainer:
            ...
            for name, param in self.model.named_parameters():
                for pattern in config.freeze_patterns:
                    if fnmatch.fnmatch(name, pattern):
                        ...
                        break
            if self.main_rank:
                self.logger.info(f'Freezed {frozen_count} parameters matching {config.freeze_patterns}')
    ...
    def run(self, config):
        ...
        for cur_epoch in range(start_epoch, config.total_epoch):
            ...
            if self.main_rank and hasattr(self, 'optimizer'):
                lr = self.optimizer.param_groups[0]['lr']
                if self.logger:
                    self.logger.info(f'Epoch {self.cur_epoch}: lr={lr:.6g}')
                if getattr(self, 'writer', None) is not None:
                    self.writer.add_scalar('train/lr', lr, self.cur_epoch)

...
from configs import MyConfig, MyConfig_a6000, load_parser
...
def test_model_speed(config, mode='cuda', ratio=0.5, imgw=2048, imgh=1024, iterations=None):
    ...
    if mode == 'cuda':
        ...
        model.eval()
        model.to(device)
        is_dual = 'dual' in config.model.lower()
        print('\n=========Speed Testing=========')
        print(f'Model: {config.model}\nEncoder: {config.encoder}\nDecoder: {config.decoder}')
        print(f'Size (W, H): {imgw}, {imgh}')

        input1 = torch.randn(1, 3, imgh, imgw).to(device)
        input2 = torch.randn(1, 3, imgh_2, imgw_2).to(device) if is_dual else None
        with torch.no_grad():
            for _ in range(10):
                if is_dual:
                    model(input1, input2)

...
HARD_CONFIG = dict(
    ...
    num_classes=1,
    use_aux=True,
    ckpt="/home/ckchng/Documents/dual_stream_two/save/bg_50_no_crop/snr_1_32_len_200/single_class/both_run2/best.pth",
    data_root="/home/ckchng/Documents/SDA_ODA/LMA_data/snr_1_32_len_200",
    split="val",
    output_dir="./val_set_vis",
    device="auto",
    batch_size=64,
    scale=1.0,
...
def discover_files(data_root: str, split: str) -> List[str]:
    """Return sorted list of basenames present in raw/<split>/."""
    raw_dir = os.path.join(data_root, "raw", split)
    return sorted(f for f in os.listdir(raw_dir) if f.endswith(".png"))


def load_tile_data(data_root: str, split: str, fname: str):
    """
    Load one tile's inputs and GT masks.

    Returns dict with:
        raw_img   – uint8 grayscale (H, W)
        raw_lbl   – uint8 binary (H, W)
    ...
    return dict(
        raw_img=_imread_gray(os.path.join(base, "raw",        split, fname)),
        raw_lbl=_imread_gray(os.path.join(base, "raw_labels", split, fname)),
...
def run_batch(model, t1, t2, num_classes: int, is_maskguided: bool):
...
def run(args):
    ...
    # --- Discover files ---
    all_files = discover_files(args.data_root, args.split)
    ...
    if args.positive_only:
        ...
        for fname in all_files:
            d = load_tile_data(args.data_root, args.split, fname)
            if d["raw_lbl"].max() > 0 or d["rt_lbl"].max() > 0:
    ...
    for batch_start in range(0, total, batch_size):
        ...
        batch_data = [load_tile_data(args.data_root, args.split, f) for f in batch_fnames]

        # Prepare tensors for model
        items = []
        for d in batch_data:
            raw_scaled = normalize_tile_to_uint8(d["raw_img"])
            spatial_3ch = make_3ch(raw_scaled)
        ...
        pred_rt_batch, pred_streak_batch = run_batch(model, t1, t2, args.num_classes, is_maskguided)
        ...
        for i, (fname, d) in enumerate(zip(batch_fnames, batch_data)):
            ...
            # The model output may be padded (multiple-of-32); crop back to GT size
            ...
            has_pred = pred_streak.max() > 0 or pred_rt.max() > 0
            if has_pred:
                out_fname = os.path.splitext(fname)[0] + "_pred.png"
                cv2.imwrite(os.path.join(args.output_dir, out_fname), composite)
        done = min(batch_start + batch_size, total)
        print(f"  [{done:5d}/{total}]  last batch IoU — streak: "
              f"{np.nanmean([v for v in iou_streak_list[-len(batch_fnames):] if not np.isnan(v)] or [float('nan')]):.3f}  "
              f"rt: {np.nanmean([v for v in iou_rt_list[-len(batch_fnames):] if not np.isnan(v)] or [float('nan')]):.3f}")
